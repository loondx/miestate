      ...
      {properties.length === 0 ? (
        <div className="rounded-lg border border-dashed border-gray-300 bg-white py-16 text-center">
          <p className="font-medium text-gray-700">
            Add your first verified project to start building trust with buyers.
          </p>
        </div>
      ) : (
          ...
          {properties.map((p) => (
            <div key={p.id} className="flex items-center gap-4 rounded-lg border border-gray-200 bg-white p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-gray-500">
                  {p.developer} · {p.locality} · {p.priceFromLabel}
                </p>
                <div className="mt-1 flex flex-wrap gap-1.5">
                  <VerifiedBadge verified={p.verified} />
                  <Badge tone={STATUS_TONE[p.status]}>{PROJECT_STATUS_LABEL[p.status]}</Badge>
                  {p.featured && <Badge tone="neutral">Featured</Badge>}
                </div>
              </div>
